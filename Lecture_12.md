**《信息论》第12讲：香农信道编码定理证明详解**

**〇、课程引言**

本节课程的核心目标是深入理解并证明克劳德·香农提出的有噪信道编码定理（香农第二定理）的可达性部分。我们将回顾该定理的声明，并详细阐述证明过程中所依赖的关键数学工具——渐进均分割特性 (AEP) 与联合渐进均分割特性 (Joint AEP)。随后，我们将系统地介绍证明的步骤，包括消息源的假设、随机编码簿的设计、基于联合典型性的译码规则，以及至关重要的差错概率分析。通过本讲，我们将理解为何在信息传输速率低于信道容量时，可以实现任意低的错误概率。
...
2.  **联合渐进均分割特性 (Joint AEP)**
    ...
    *   **联合典型集 $A_{\epsilon}^{(n)}(X,Y)$:**
        一个序列对 $(x^n, y^n)$ 被称为联合 $\epsilon$-典型，如果它同时满足：
...
3.  **译码规则 (Decoding Algorithm/Rule)** (L12P2, L12P3)
    ...
    *   **译码判决：**
        *   **译码成功 (Decoding Success):**
            ...
            (L12P3 板书参考 (右侧黑板第3点 Decoding Alg (rule) 的 (c) 项): `(c) if there is a unique $c_i=(x_1, \ldots, x_n)$ forming jointly typical w/ $y_1, \ldots, y_n$. V`)
            (注：译码成功隐含要求接收到的序列 $\mathbf{y}$ 本身是关于其边际分布 $P(Y)$ 的典型序列，因为如果 $\mathbf{y}$ 非典型，它不可能与任何 $\mathbf{x}_i$ 构成联合典型序列。)

        *   **译码失败 (Decoding Failure):** (L12P3)
            *   **情况 (a) - 无联合典型码字:** 如果在编码簿中找不到任何一个码字 $c_j$ 使得 $(\mathbf{x}_j, \mathbf{y})$ 是联合典型的，则译码失败。
                (L12P3 板书 `(a) if no such $x_1 \ldots x_n$, report fail.`)
            ...
            *   **情况 (b) - 多于一个联合典型码字:** 如果找到了多个（不止一个）码字 $c_j, c_k, \ldots$ ($j \neq k$) 都与接收到的 $\mathbf{y}$ 构成联合典型序列，译码器无法唯一确定发送的是哪个消息，则译码失败。
                (L12P3 板书 `(b) if there are more than one codewords jointly typical w/ $(y_1, \ldots, y_n)$, report fail.`)

4.  **差错概率分析 (Error Probability Analysis)** (L12P3)
    ...
    *   **分析 $P(E_1)$ (对应差错类型 (a) 的概率):**
        ...
        *   (L12P3 教授解释 03:30-03:57): "因为这一个真实的输入和输出...它们就是按照 $P(X,Y)$ 那个 joint probability distribution generate 出来的。那么就会以接近1的 probability，它们就是一个 jointly typical sequence。"
    ...
    *   **分析 $P(E_2)$ (对应差错类型 (b) 的概率):**
        ...
        *   **结论:** 当 $R < C$ 时，$C-R > 0$，所以 $P(E_2)$ 随着 $n \to \infty$ 而指数级趋向于 $0$。
            (L12P3 板书 $P(b \text{ occurs}) \le (2^{nR}-1) \cdot 2^{-nI(X;Y)} \approx 2^{nR} \cdot 2^{-nI(X;Y)} = 2^{-n(I(X;Y)-R)}$，然后是 $\approx 2^{-n(C-R)}$，并指出 If $R < C$, then $C-R > 0 $, so $P(b \text{ occurs}) \to 0 $ as $ n \to \infty $)
