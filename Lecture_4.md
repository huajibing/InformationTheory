...
1.  **定义**:
    ...
    $H(X,Y) = -\sum_{i}\sum_{j} P(X=x_i, Y=y_j) \log_2 P(X=x_i, Y=y_j)$ (比特)
    也可以简洁地写作：
    $H(X,Y) = -\sum_{x \in \mathcal{X}, y \in \mathcal{Y}} P(x,y) \log_2 P(x,y)$ (比特)
    ...
    *   **独立性**: 如果 $X$ 和 $Y$ 是相互独立的 (independent)，即 $P(X,Y) = P(X)P(Y)$，则：
        ...
        *推导*:
        ...
        $= -\sum_x P(x) \log_2 P(x) \sum_y P(y) - \sum_y P(y) \log_2 P(y) \sum_x P(x)$
         যেহেতু $\sum_y P(y) = 1$ 和 $\sum_x P(x) = 1$:
...
1.  **特定条件下的条件熵**:
    ...
    $H(Y|X=x_i) = -\sum_j P(Y=y_j|X=x_i) \log_2 P(Y=y_j|X=x_i)$ (比特)
...
2.  **平均条件熵 (Conditional Entropy)**:
    ...
    $H(Y|X) = -\sum_i \sum_j P(X=x_i, Y=y_j) \log_2 P(Y=y_j|X=x_i)$ (比特)
...
4.  **互信息的概率分布表达式 (课堂习题)**
    ...
    *   **推导思路**: 从 $I(X;Y) = H(X) + H(Y) - H(X,Y)$ 出发，将各个熵的定义代入。
        ...
        $= \sum_{i,j} P(X=x_i, Y=y_j) \log_2 \frac{P(X=x_i, Y=y_j)}{P(X=x_i)P(Y=y_j)}$ (比特)
...
1.  **联合熵 (Joint Entropy) 的推广**:
    ...
    $H(X_1, X_2, \dots, X_n) = -\sum_{x_1, \dots, x_n} P(x_1, x_2, \dots, x_n) \log_2 P(x_1, x_2, \dots, x_n)$ (比特)
...
2.  **KL 散度的定义**:
    ...
    $D(P||Q) = \sum_i p_i \log_2 \frac{p_i}{q_i}$ (比特)
    (也常记作 $KL(P||Q)$)
