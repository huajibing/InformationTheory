...
*   **回顾熵与平均码长下界 (源自 Lecture_3_Part_1)：**
    在讨论实际编码设计之前，我们曾推导过，对于一个离散随机变量 $X$，其概率质量函数 (PMF) 为 $P=(p_1, \dots, p_n)$，若允许码长 $L_i$ 为实数，并且编码是前缀无关的（满足 Kraft 不等式 $\sum 2^{-L_i} \le 1$），则最小化平均码长 $\sum p_i L_i$ 的理论下界是信源的**熵 (Entropy)** $H(X)$：
    $H(X) = \sum_{i=1}^{n} p_i \log_2 \frac{1}{p_i} = -\sum_{i=1}^{n} p_i \log_2 p_i$ (单位: 比特)
    这个结论是通过以下优化问题得到的：
    ```latex
    \min_{L_1, ..., L_n} \sum_{i=1}^{n} p_i L_i
    \text{s.t. } \sum_{i=1}^{n} 2^{-L_i} \le 1 \quad (\text{后被强化为 } =1)
    \quad \quad L_i > 0 \quad (\text{理论分析时放宽为实数})
    ```
...
3.  **优化目标：最小化平均码长 (Minimize Average Code Length)**
    ...
    *   我们的目标是使 $\bar{L}$ 最小。
        *板书 (1:40 - 1:57, Part_2/3):*
        ```latex
        \text{Goal: } \sum P_i l(c_i) \text{ is minimum.}
        ```
        *(注：教师在板书中常使用 $|C_i|$ 来代表码字 $c_i$ 的长度 $l(c_i)$。)*

4.  **最优码 (Optimal Code):**
    *   使平均码长 $\bar{L}$ 最小的编码方案称为**最优码**。
        *板书 (2:25, Part_3):* `optimal code`
...
1.  **性质1：概率与码长的单调非增关系 (Monotonicity)**
    ...
    *   *板书 (5:38, Part_2/3):*
        ```latex
        \text{Properties of Optimal Code}
        \text{1. Assume } p_1 \ge p_2 \ge \dots \ge p_n \implies \text{optimal code must satisfy } |c_1| \le |c_2| \le \dots \le |c_n|
        ```
        *(这里的 $|c_i|$ 即 $l(c_i)$)*

2.  **性质2：最优前缀码的Kraft不等式取等 (Kraft Inequality holds with equality for optimal prefix-free codes)**
    *   **结论：** 对于一个最优的前缀无关码，其码长 $l(c_i)$ 必须使 Kraft 不等式取等号：
    ...
    *   *板书 (6:20 - 6:30, Part_2/3):*
        ```latex
        \text{2. Kraft's Ineq. } \sum_{i=1}^{n} 2^{-|c_i|} = 1 \quad \text{for opt. code}
        ```
...
3.  **性质3：最长码字的特性 (Properties of Longest Codewords)**
    ...
    *   *板书 (7:10 - 7:28, Part_3):*
        ```latex
        \text{3. } |C_n| = |C_{n-1}| \implies C_n, C_{n-1} \text{ are sibling nodes.}
        ```
...
1.  **熵 (Entropy) 的多角度理解**
    *   **定义重申：** $H(X) = \sum_{i=1}^{n} p_i \log_2 \frac{1}{p_i}$ (比特)。
        *板书 (05:09 - 05:13, Part_1):*
        ```latex
        \text{Entropy}
        \text{r.v. } X \quad \text{pmf } P=(p_1, ..., p_n)
    ...
    *   **平均信息量 (自信息量的期望)：**
        ...
        *板书 (07:08 - 07:19, Part_1):*
        ...
        \text{Information }
        \begin{cases}
 \text{r.v. } X & H(X) \quad (\text{bits}) \\
 \text{message } m_i & \log_2 \frac{1}{p_i} \quad (\text{bits})
            \end{cases}
        ```
...
2.  **熵的可加性 (Additive Property of Entropy / Chain Rule for Entropy - 特殊情况)**
    ...
    *   **场景设定：**
        ...
        *板书 (23:45 - 24:03 & 24:40, Part_1):*
        ```latex
        \text{r.v. X: } \quad P_1 \quad P_2 \quad \dots \quad P_{n-1} \quad P_n (=q_1+q_2)
        \text{Y (given } X=x_n \text{): pmf of Y is } (\frac{q_1}{P_n}, \frac{q_2}{P_n})
        \text{r.v. Z: } \quad P_1 \quad P_2 \quad \dots \quad P_{n-1} \quad q_1 \quad q_2
        ```
    *   **可加性公式：**
